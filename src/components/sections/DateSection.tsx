...
const DateSection = ({ bgColor = 'white' }: DateSectionProps) => {
    ...
    useEffect(() => {
        const calculateTimeLeft = () => {
            ...
            const isPassed = difference <= 0;
            const timeDifference = Math.abs(difference);
        ...
        <DateSectionContainer $bgColor={bgColor}>
            ...
            <CountdownContainer>
                ...
                <CountdownWrapper>
                    ...
                    <VerticalDivider />
                    <CountdownItem>
                        <CountdownValue>{timeLeft.hours < 10 ? `0${timeLeft.hours}` : timeLeft.hours}</CountdownValue>
                        <CountdownLabel>시간</CountdownLabel>
                    </CountdownItem>
                    <VerticalDivider />
                    <CountdownItem>
                        <CountdownValue>{timeLeft.minutes < 10 ? `0${timeLeft.minutes}` : timeLeft.minutes}</CountdownValue>
                        <CountdownLabel>분</CountdownLabel>
                    </CountdownItem>
                    <VerticalDivider />
                    <CountdownItem>
                        <CountdownValue>{timeLeft.seconds < 10 ? `0${timeLeft.seconds}` : timeLeft.seconds}</CountdownValue>
...
const DayName = styled.div<{ $isWeekend?: string }>`
    color: ${(props) => (props.$isWeekend === 'sun' ? '#e57373' : props.$isWeekend === 'sat' ? '#64b5f6' : 'inherit')};
    font-size: 0.875rem;
